...
def make_key(
    ...
    *,
    ...
    if write_task_id is not None and write_idx is not None:
        ...
        sk = f"{checkpoint_ns}#{checkpoint_id}#write#{write_task_id}#{write_idx:010d}"
    else:
        # Key for checkpoints
        sk = (
            f"{checkpoint_ns}#{checkpoint_id}#checkpoint"
            if checkpoint_id
            else f"{checkpoint_ns}#"
        )
...
def validate_checkpoint_item(item: Dict[str, Any]) -> CheckpointItem:
    ...
    if not item['SK'].endswith('#checkpoint'):
        raise DynamoDBValidationError(
            f"Invalid checkpoint SK format: {item['SK']}"
        )
...
def validate_write_item(item: Dict[str, Any]) -> WriteItem:
    ...
    if '#write#' not in item['SK']:
        raise DynamoDBValidationError(
            f"Invalid write SK format: {item['SK']}"
        )
